...
make -C "${KDIR}" M=${PWD} ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} modules >> $LOGFILE 2>&1
echo "[*] Build done: $(pwd)/cpufreq_fast.ko"
modinfo cpufreq_fast.ko | grep vermagic || true